...
def getincident():
    ...
    conn = psycopg2.connect("dbname=jack user=jack")
    cur = conn.cursor()
    ...
    for incident in incidents[1:num_inci]:
        ...
        cur.execute(
            "INSERT INTO incidents (prime_street, cross_street, time, eid, etype, alarm, beat, units, address, latitude, longitude) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", to_db)
    conn.commit()
    #id_new_row = cur.fetchone()[0]
    print update
    #print "Last Row Inserted: " + id_new_row
    # IT WORKS MAFACKA!
    time.sleep(180)
